...
|--------|------|---------|---------|------------------|--------|
| SCR-sb-001 | キャンバス本体 | REQ-sb-002,003,005〜011 | — | CMP-sb-001〜008 | 同一 |
...
機能→クラス→DB の関係をノードグラフとアウトラインで把握し、詳細はインスペクタで確認する。設計プロジェクトは常に1件だけ表示する。
...
| 左 | アウトライン（機能チェック＋ツリー） | 機能マルチ選択・ノード選択 |
...
+--------+----------------------------------------+----------------+
| アウトライン|  キャンバス（選択機能の部分グラフ） | インスペクタ   |
| □ lobby  |  Feature(s) → Class → Table         | 選択ノード名  |
| □ race   |  継承: Class -.-> Class（破線）       | 詳細・ソース  |
|   CLS-.. |                                      | 関連→ジャンプ |
|              [ -  100%  + ]  lock                |               |
+--------+----------------------------------------+----------------+
```
...
| 操作 | 条件 | 結果 |
|------|------|------|
| プロジェクト切替 | projects あり | 表示スコープ切替、`?project=` 更新 |
| 機能チェック | アウトライン | キャンバスを部分グラフに更新 |
| ノードクリック / タップ | ノードあり | インスペクタ更新、`?node=` 更新 |
| ダブルクリック | デスクトップ・ノードあり | 中心フィット、関連ハイライト |
| 検索入力 | — | アウトラインとキャンバスを絞り込み |
| ギャップのみ | — | 未実装ノードのみ表示 |
| 層フィルタ | — | 列の表示切替 |
